...
> 법령 검증 출처: `reference/소방시설법 및 화재예방법령집.pdf`, 법제처 OpenAPI/국가법령정보센터 원문 (고객 BYOK 스냅샷 수집 전용)
...
| **v0.2.3** | **2026-05-26** | **데이터베이스 기준선 보강**. ① 별도 DB 설계 문서 추가: [fire-inspection-system.database.md](./fire-inspection-system.database.md) ② 운영 DB 후보를 PostgreSQL 호환 RDB(Supabase Postgres 등)로 명시 ③ 구현 순서에 `데이터 저장소/테이블 설계` 단계를 step 3으로 삽입하고 이후 단계 번호 조정 | `packages/types`와 `packages/fire-data` 구현 전에 orgId, JSON 스냅샷, 첨부 메타, 관계형 테이블 경계를 확정해야 이후 마이그레이션 비용이 낮음. |
| **v0.2.4** | **2026-05-27** | **법령 데이터 접근 전략 변경**. 해외 웹 런타임의 법제처 API 직접 호출을 폐기하고, 저장된 법령 스냅샷만 런타임에서 조회하도록 변경. `LAW_DATA_MODE=runtime` 분리와 miss 시 `확인 필요` 차단 정책 추가. | 해외 웹서버에서 국내 법령정보 API 접근이 막히거나 불안정할 수 있으므로 사용자 요청 경로에서 공식 API 의존을 제거. |
| **v0.2.5** | **2026-05-27** | **BYOK 법령 스냅샷 수집 확정**. 웹앱은 Vercel 우선(`icn1`, 불가 시 `hnd1`/`sin1`), DB/Auth/Storage는 Supabase Seoul 우선. 고객 관리자가 자기 법제처 API 키로 필요한 시행일 스냅샷을 생성·업로드하고, 설비 추천은 우리 룰 엔진이 수행. | 개발 운영사의 과거 법령 선제 구축 부담을 줄이면서, 설비 추천의 품질과 재현성은 우리 룰 엔진으로 통제하기 위함. |
...
| 3 | v1 저장소 | **로컬 JSON** (현 위치 `data/`) + **PostgreSQL 호환 테이블 기준선 사전 설계** ([DB 설계](./fire-inspection-system.database.md)) |
| 4 | 법령 시계열 전략 | **고객 키 기반 스냅샷 수집(BYOK)**. 고객 관리자가 필요한 시행일 스냅샷을 생성·업로드하고, 단계 A는 저장된 스냅샷만 조회해 `WorkOrder`에 동결 |
| 4A | 배포·임대 | **Vercel + Supabase Seoul**. 서버에는 법제처 API 키를 저장하지 않음 |
...
interface WorkOrder {
  ...
  applicableLawSnapshot: LawCitation[]; // 저장된 법령 스냅샷 동결본
...
Phase 4에서 `packages/types/src/forms.ts`에 다음 derive 타입을 둔다. 별지 PDF 정밀 매핑은 Phase 13/14에서 채운다.
...
│  ├─ law-client/                                  # 저장된 스냅샷 조회 + 고객 BYOK 수집
...
│  ├─ law-snapshots/                               # 고객 BYOK 수집으로 만든 법령 스냅샷
│  │  └─ {law-id}-{year}.json
│  └─ projects/                                    # v1 로컬 저장소
│     └─ {projectId}/
...
>
> **공통 원칙**: 모든 함수는 **저장된 법령 스냅샷·정적 데이터·결정론적 룰**만 사용. LLM 호출 0. 환각 차단 원칙 유지.
...
```typescript
interface LawClient {
  // 런타임 기본 경로. 해외 웹서버에서도 동작해야 하므로 네트워크 호출 0.
  lookupVerifiedSnapshot(input: {
    lawId: string;
    atDate: string;
    articleNo?: string;
  }): Promise<LawCitation[]>;

  // 고객 관리자 BYOK 수집 세션 전용. 키는 저장하지 않는다.
  fetchLawSnapshotWithUserKey(input: {
    lawId: string;
    atDate: string;
    apiKey: string;
  }): Promise<RawLawPayload>;
}
```

해외 웹서버는 법제처 API를 직접 호출하지 않는다. 고객 관리자가 자기 법제처 API 키로 필요한 스냅샷을 생성·업로드하고, 런타임은 저장된 스냅샷만 조회한다. 고객이 생성하는 것은 설비 리스트가 아니라 법령 스냅샷이며, 설비 섹션 추천은 우리 룰 엔진이 수행한다.

스냅샷 메타데이터 최소 필드:

```typescript
interface LawSnapshotMeta {
  snapshotId: string;
  lawId: string;
  lawName: string;
  ...
  verifiedBy?: string;
  contentHash: string;
  status: 'raw' | 'normalized' | 'verified' | 'deprecated';
  collectedByOrgId?: string;
  collectionMode: 'customer-byok' | 'admin-upload';
}
```

폴백 정책:

- 구조 검증을 통과한 스냅샷이 있으면 즉시 반환하고 작업지시에 `snapshotId`와 `contentHash`를 동결한다.
- 같은 날짜의 스냅샷이 없으면 고객 관리자에게 스냅샷 생성을 요청한다.
- 법적 판정에 필요한 구간이 누락되거나 구조 검증에 실패하면 작업지시 동결을 막고 `확인 필요`를 표시한다.
- `LAW_DATA_MODE=runtime`에서 공식 API 호출 함수가 실행되면 예외를 던진다.

### 5.3A 배포·런타임 환경

| 환경 | 위치 | 주요 환경변수 | 역할 |
|---|---|---|---|
| Web runtime | Vercel `icn1` 우선, 대안 `hnd1`/`sin1` | `LAW_DATA_MODE=runtime` | Next.js 화면/API, 저장된 스냅샷 조회, 작업지시 동결 |
| DB/Auth/Storage | Supabase Seoul `ap-northeast-2` | Supabase URL/anon/service role | 운영 데이터, 첨부, 생성 PDF 저장 |
| Customer BYOK collection | 고객 관리자 브라우저 세션 | 사용자가 입력한 법제처 API 키 | 법제처 `eflaw`/`eflawjosub` 조회, 스냅샷 JSON 생성 |
| Snapshot import | 관리자 화면 업로드 | 로그인 세션 | 스냅샷 저장/갱신 |

제약:

- Vercel 파일시스템은 영구 저장소로 사용하지 않는다.
- Vercel/Supabase에는 고객 법제처 API 키를 저장하지 않는다.
- 법령 스냅샷 생성 실패는 STEP 4의 보류 상태로 처리하고, 기존 작업지시 데이터는 변경하지 않는다.
- 대량 PDF/OCR 처리는 실제 수요 확인 후 v1.5+에서 재검토한다.
...
| 항목 | v1 PoC 기준 |
|---|---|
| 응답 속도 (단계 A 작업지시 생성) | 5초 이내 (저장된 스냅샷 조회 기준, 공식 API 호출 0) |
...
3. **데이터 저장소/테이블 설계 (v0.2.3 신설)**: 운영 DB는 PostgreSQL 호환 RDB 기준. `organizations`, `users`, `customer_companies`, `buildings`, `inspection_projects`, `building_registers`, `suggested_facility_sections`, `work_orders`, `inspections`, `inspection_sections`, `inspection_results`, `field_notes`, `discrepancies`, `attachments`, `billing_documents`, `billing_line_items` 기준선 확정. 상세는 [DB 설계](./fire-inspection-system.database.md).
4. **packages/types**: §3.1 도메인 타입 전부 코드화 (v0.2 신설 엔티티 포함). `apps/web/lib/auth/guard.ts` 단일 role 가드.
5. **packages/fire-data**: 기존 `data/fire-duty-master.json` 로더 + `inspection-checklist.json` 신규 작성 (PoC 7종)
6. **packages/law-client**: 저장된 스냅샷 조회 + 고객 BYOK 법제처 API 수집 (먼저 mock으로 시작)
7. **packages/ocr-client (v0.2)**: `manual` provider만 먼저 구현 (빈 extraction 반환). 실제 OCR 엔진은 §10 Open Question 해소 후 추가.
8. **apps/web/lib/domain/ 6함수** 구현: `extractBuildingRegister`, `lookupLawByYear`, `suggestFacilities`, `deriveInspectionScope`, `composeWorkOrder`, `validateInspection`
9. **apps/web 단계 A-1 UI**: 로그인(admin/field) + CustomerCompany/Building/InspectionProject 생성 — §6A 토큰 적용
10. **apps/web 단계 A-2 UI (v0.2)**: 건축물대장 업로드 → OCR 호출 → **검수 화면**(원본 PDF vs 추출 필드 나란히, FieldDiffBadge 표시) → confirm
11. **apps/web 단계 A-3 UI (v0.2)**: `suggestFacilities` 실행 → 추천 설비 검토 화면 (included/excluded/modified/pending)
12. **apps/web 단계 A-4 UI**: 작업지시 검토 + 동결 (composeWorkOrder + hash)
13. **별지 9호 HTML**: 8쪽 정적 레이아웃 + `Form9Data` + `WorkOrder` 바인딩 + `/print/form9` 라우트 (§6 인쇄 표준만 적용, §6A 컬러 비적용)
14. **별지 4호 HTML**: 점검표 7종 정적 레이아웃 + 점검번호 키(`1-A-001` 체계) 바인딩
15. **apps/web 단계 B UI**: 점검 홈(배정 섹션) → 설비별 점검 입력(InspectionResult + FieldNote: 메모/사진/음성/파일) + Discrepancy 로그
16. **비용 문서 (v0.2)**: BillingDocument 입력 + `BillingInvoice.tsx` + `/print/billing/[billingId]` PDF
17. **인쇄 검증**: Chromium headless로 PDF 생성 + 좌표 자동 비교 (form9·form4·billing 3종)

## 10. Open Questions (Design 단계 종료 시 해소 필요 · v0.2 갱신)

### v0.1부터 미해소 (계속 추적)

- [x] 법제처 OpenAPI 키 기본 동작 확인: `target=law`, `target=eflaw`, `target=eflawjosub` 조회 성공 (2026-05-27 로컬 테스트)
- [ ] 고객 키 기반 법령 스냅샷 생성/업로드 UX 결정: API 키 입력 후 자동 생성까지 할지, snapshot JSON 업로드를 v1 우선으로 둘지
